...
- `public/assets/characters/leo/leo sprite sheet.png`, `public/assets/characters/alice/ALICE SPRITE SHEET.png` — sheets de animação completos (IDLE, CAMINHADA, CORRIDA, PREPARE, FOREHAND, BACKHAND, SMASH, MISS, COMEMORAÇÃO). **Fatiados** por componente conectado do canal alpha (ver `derived/FRAME_MAP.md` de cada personagem): em uso IDLE, CAMINHADA, CORRIDA, PREPARAÇÃO, contato, MISS e COMEMORAÇÃO (frames 1–2). SMASH/SAQUE/VOLEIO seguem não usados (poses sobrepostas na sheet).
...
- `public/assets/efeitos/efeitos sprite sheet.png` — banners de VFX. **Em uso** desde o polish pass: PERFECT!/GOOD!/LATE!/MISS! (sobre a cabeça de quem bateu) e POINT!/GAME!/SET!/MATCH! (centro da quadra), extraídos por chroma-key para `efeitos/derived/` e desenhados no canvas por `components/GameCanvas.tsx`. Os demais painéis da sheet (rastros, trajetórias especiais, meteorologia etc.) seguem sem uso.
